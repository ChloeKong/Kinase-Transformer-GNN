...
bond_type_dict = {'SINGLE': 0, 'DOUBLE': 1, 'TRIPLE': 2, 'AROMATIC': 3, 'virtual_bond': 4}
...
def bond_feature(mol,
    ...
    for bond in mol.GetBonds():
        ...
        row += [start, end]
        col += [end, start]
        # 得到类似[0,0,1,1,0,0,2,2,3,3]的列表，之后要经过one-hot再把每一位拓展成一个四位的向量
        bond_idx += 2 * [bond_type_dict[str(bond.GetBondType())]]# 这里是列表扩展，不是每个数乘以2
        if stereo == True:
    ...
    if fully_connect == True:
        ...
        virtual_idx= [4] * length # 没有键作为第五类（从零开始）
    ...
    # 此时得到的edge_index按键排序的，同一个键的两个原子在一起先后出现
    edge_index = torch.tensor([row, col], dtype=torch.long)

    # 根据bond_idx生成one-hot边特征
    _ = []
